...
if 'score_data' not in st.session_state:
    st.session_state.score_data = pd.DataFrame(columns=["날짜", "이름", "반", "점수"])
...
with st.form("score_form"):
    ...
    if submitted:
        new_row = pd.DataFrame([[date, name, class_name, score]], columns=["날짜", "이름", "반", "점수"])
        st.session_state.score_data = pd.concat([st.session_state.score_data, new_row], ignore_index=True)
        st.success(f"{name} 학생의 점수가 저장되었습니다!")
...
if not st.session_state.score_data.empty:
    ...
    ranked = latest_scores.sort_values(by=["반", "점수"], ascending=[True, False])
    ranked['순위'] = ranked.groupby("반")['점수'].rank(ascending=False, method='min')
    st.subheader(f"📅 {latest_date.strftime('%Y-%m-%d')} 기준")
    st.dataframe(ranked[["반", "순위", "이름", "점수"]].sort_values(by=["반", "순위"]))
